...
public partial class MainViewModel
{
    ...
    private async Task LoadAppSettingsAsync()
    {
        ...
        {
            UpdateStatusUi($"Failed to load app settings: {ex.Message}", false);
        }
    }
    ...
    private async Task LoadWorkspaceStateAsync()
    {
        ...
        {
            UpdateStatusUi($"Failed to restore workspace state: {ex.Message}", false);
        }
    }

    // Renamed from LoadGenerationPresets() to follow async naming convention.
    // The old name is kept as a non-async shim so callers that don't await
    // (e.g. RenameGenerationPresetAsync) can still compile without changes.
    private async Task LoadGenerationPresetsAsync()
    {
        try
        {
        ...
        {
            UpdateStatusUi($"Failed to load generation presets: {ex.Message}", false);
        }
    }

    // Shim so existing call-sites that use the old name still compile.
    private void LoadGenerationPresets() => _ = LoadGenerationPresetsAsync();
    ...
    private async Task LoadServersAsync()
    {
        ...
        {
            UpdateStatusUi($"Failed to load servers: {ex.Message}", false);
        }
    }
    ...
    private async Task LoadHistoryAsync()
    {
        ...
        {
            UpdateStatusUi($"Failed to load history: {ex.Message}", false);
        }
    }
